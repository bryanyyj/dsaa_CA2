...
# def restore_line_all_matches(line, trie):
#     """
#     For each word in the line, if it contains '*',
#     find all possible matches in the trie and replace.
#     Returns a list of possible restored lines.
#     """
#     words = line.strip().split()
#     # Start with one possible line (empty)
#     possibilities = [[]]

#     for word in words:
#         if '*' in word:
#             matches = wildcard_match(trie, word)
#             if matches:
#                 # Build new possibilities by branching
#                 new_possibilities = []
#                 for base in possibilities:
#                     for match, _ in matches:
#                         new_possibilities.append(base + [match])
#                 possibilities = new_possibilities
#             else:
#                 # No matches — keep the word as is
#                 for base in possibilities:
#                     base.append(word)
#         else:
#             # Normal word, just add it
#             for base in possibilities:
#                 base.append(word)

#     # Convert lists of words back to lines
#     return [' '.join(words) for words in possibilities]
